...
event beginsi(OutPort).
event endsi.
event beginse(OutPort).
event sanitysi.
event sanityse.
event sanityc.
event error.
...
local_route_integrity_1 = """
(**** Local Routes Integrity ****) (* Those properties are trivially verified within our model *)
query
  xegr: OutPort;
  event(beginse(xegr)) ==> event(endsi).
"""

local_route_integrity_i = """
(**** Local Routes Integrity ****) (* Those properties are trivially verified within our model *)
query
  xegr: OutPort;
"""

transitional_route_integrity = """
(**** Transitional Routes Integrity ****)
query xegr: OutPort;
  event(endsi) ==> event(beginsi(xegr));
"""
...
for i in range(1, n + 1):
    with open('SDNsec_route_' + str(i) + '.pv', 'w+') as f:
        f.write(preambule)
        for k in range(1, i):
            f.write("event begins" + str(k) + "(OutPort).\n")
            f.write("event ends" + str(k) + ".\n")
        ...
        if i == 1:
            f.write(local_route_integrity_1)
        else:
            f.write(local_route_integrity_i)
            for j in range(1, i + 1):
                if j == 1:
                    f.write(" event(begins1(xegr)) ==> event(endsi);\n")
                elif j == i:
                    f.write(" event(beginse(xegr)) ==> event(ends" + str(i - 1) + ").\n")
                else:
                    f.write(" event(begins" + str(j) + "(xegr)) ==> event(ends" + str(j - 1) + ");\n")
        f.write(transitional_route_integrity)
        ...
        if i == 1:
            ...
            f.write("    let MACe = MAC(ske, (egre, FEi, B, h(p, PVFi, n_s0))) in\n")
            ...
            for j in range(1, i + 1):
                if j == 1:
                    ...
                    f.write("    let MAC1 = MAC(sk1, (egr1, FEi, B, h(p, PVFi, n_s0))) in\n")
                    f.write("    let FE1 = (egr1, MAC1) in\n")
                elif (j == i):
                    first_message_plaintext = first_message_plaintext + ", egre, MACe"
                    f.write("    let PVFe = MAC(ske, (PVF" + str(i - 1) + ", F, n_s0 + " + str(i) + ")) in \n")
                    f.write(
                        "    let MACe = MAC(ske, (egre, FE" + str(i - 1) + ", B, h(p, PVF" + str(i - 1) + ", n_s0 + " +
                        str(i - 1) + "))) in\n"
                    )
                    f.write("    let FEe = (egre, MACe) in\n")
                else:
                    ...
                    f.write(
                        "    let PVF" + str(j) + " = MAC(sk" + str(j) + ", (PVF" + str(j - 1) + ", F, n_s0 + " +
                        str(j) + ")) in \n"
                    )
                    f.write(
                        "    let MAC" + str(j) + " = MAC(sk" + str(j) + ", (egr" + str(j) + ", FE" + str(j - 1) +
                        ", B, h(p, PVF" + str(j - 1) + ", n_s0 + " + str(j - 1) + "))) in\n"
                    )
        ...
        if i == 1:
            ...
            for j in range(1, i + 1):
                if j == 1:
                    ...
                    receive_message_plaintext = receive_message_plaintext + ", xegr" + str(j) + ": OutPort, MAC" + str(
                        j
                    ) + ": bitstring"
        ...
        if i == 1:
            ...
            f.write("    if (MACe = MAC(ske, (xegre, B, B, h(p, PVFp, n_s)))) then\n")
            f.write("         event endsi;\n")
            f.write("         let n_s' = n_s + 1 in\n")
            f.write("         let PVFe = MAC(ske, (PVFp, F, n_s')) in\n")
            f.write("         event beginse(xegre);\n")
            ...
            for j in range(1, i + 1):
                if j == 1:
                    ...
                    f.write("    in(c, " + packet_entete + packet + "));\n")
                    f.write("    let B = (F, t) in \n")
                    f.write("    if (MAC1 = MAC(sk1, (xegr1, B, B, h(p, PVFp, n_s)))) then\n")
                    f.write("        event endsi;\n")
                    f.write("        let n_s' = n_s + 1 in \n")
                    f.write("        let PVF1 = MAC(sk1, (PVFp, F, n_s')) in \n")
                    f.write("        event begins1(xegr1); \n")
                    ...
                    f.write("    if (MACe = MAC(ske, (xegre, FE" + str(i - 1) + ", B, h(p, PVFp, n_s)))) then\n")
                    f.write("        event ends" + str(i - 1) + ";\n")
                    f.write("        let n_s' = n_s + 1 in \n")
                    f.write("        let PVFe = MAC(ske, (PVFp, F, n_s')) in \n")
                    f.write("        event beginse(xegre); \n")
                    f.write("        event sanityse;\n")
                    ...
                    f.write(
                        "    if (MAC" + str(j) + " = MAC(sk" + str(j) + ", (xegr" + str(j) + ", FE" + str(j - 1) +
                        ", B, h(p, PVFp, n_s)))) then\n"
                    )
                    f.write("        event ends" + str(j - 1) + ";\n")
                    f.write("        let n_s' = n_s + 1 in \n")
        ...
        f.write(
            "    (!ProcessController(ski, ske " + keys_controller_main + ")) |  (!ProcessSi(ski)) | (!ProcessSe(ske))"
        )

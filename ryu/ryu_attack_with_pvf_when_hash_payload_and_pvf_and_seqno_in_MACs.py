#SCENARIO:  WE ARE INTEGRATING THE HASH OF THE PVF IN THE MAC
#THE ROUTE IS SUPPOSED TO BE S0-->S1-->S2
#WE WILL FORWARD THE PACKET FROM S0-->S12-->S2 BYPASSING S1
#S12 WILL NOT CHANGE ANYTHING.
#THE RESULT: S12 WILL DETECT THAT ITS MAC CALCULATED DIFFERENT FROM THE ONE IN THE PACKET RECEIVED DOES NOT CONTAIN PVF(S1)
...
def hash_payload_pvf(payload, pvf,seqno):
    payload_encoded = payload
    pvf_str = str(pvf)
    pvf_encoded = pvf_str.encode('utf-8')
    seqno_str=str(seqno)
    seqno_encoded = seqno_str.encode('utf-8')
    combined = payload_encoded + pvf_encoded + seqno_encoded
    ...
    def packet_in_handler(self, ev):
        ...
        if ethertype == 0x88B6 :
            if dpid == 1: #s0
                ...
                egress_s0=3     #here when you keep egress_s0=2 it's the legitimate route, we changed it to 3 so S0 can forward it to S12 not S1
                ...
                if ethertype == 0x88B6 :
                    ...
                    hash_p_pvf=hash_payload_pvf(payload,pvf_s0,seq_no) #MODIFICATION
                
                    x = str(egress_s1)+FE_s0+str(custom_header.flow_id)+str(custom_header.exp_time)+hash_p_pvf #MODIFICATION
                    hmac_s1_calculated_by_s1=compute_hmac_fe(x,self.keys[dpid])
                    print(f"hmac_s1_calculated_by_s1:{hmac_s1_calculated_by_s1}")
                ...
                if ethertype == 0x88B6 :
                    ...
                    hash_p_pvf=hash_payload_pvf(payload,pvf_s1,seq_no) #MODIFICATION
                    x = str(egress_s2)+FE_s1+str(custom_header.flow_id)+str(custom_header.exp_time)+hash_p_pvf #MODIFICATION
                    
                    hmac_s2_calculated_by_s2=compute_hmac_fe(x,self.keys[dpid])
                    print (f"hmac_s2_calculated_by_s2:{hmac_s2_calculated_by_s2}")
                ...
                print("\033[31m"+"I am the malicious switch S12 !! I have got the packet : "+"\033[0m")
                print("\033[33m"+str(msg.data)+"\033[0m")
                msg = ev.msg
                dp = msg.datapath
                pkt = packet.Packet(msg.data)
                eth_pkt = pkt.get_protocol(ethernet.ethernet)
                ...
                if ethertype == 0x88B6 :
                    ...
                    custom_header_data = data[14:52]
                    payload = data[52:]
                    payload = (b'XX XXXX' * 99)[:798] #(b'L3 Data' * 99)[:798]  #MODIFICATION
                    # Unpack the custom header data
                    custom_header = struct.unpack(custom_pack_str, custom_header_data)
                    ...
                    eth_header = struct.pack('!6s6sH', bytes.fromhex(eth_pkt.dst.replace(':','')), bytes.fromhex(eth_pkt.src.replace(':','')),eth_pkt.ethertype)
                    new_packet_data = eth_header + updated_header_bytes + payload
                    print("\033[31m"+"I will change the payload of (L3 data) to (XXXXXX) !!"+"\033[0m") 
                    self.forward_packet(dp, msg, new_packet_data, 2)                       
            else:
                print("!! HEY : dpid is not included in the expected route")  
